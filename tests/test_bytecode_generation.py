...
def compile_and_get_bytecode_and_constants(source):
    ...
    return result.chunk.code, result.chunk.constants

def test_number_addition_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: number = 1; var b: number = 2; var c = a + b;")
    assert OpCode.OP_ADD_NUMBER in bytecode
    assert OpCode.OP_ADD not in bytecode

def test_string_concatenation_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants('var a: string = "hello"; var b: string = " world"; var c = a + b;')
    assert OpCode.OP_ADD_STRING in bytecode
    assert OpCode.OP_ADD not in bytecode

def test_any_addition_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: any = 1; var b: any = 2; var c = a + b;")
...
def test_number_subtraction_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: number = 2; var b: number = 1; var c = a - b;")
...
def test_number_multiplication_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: number = 2; var b: number = 3; var c = a * b;")
    assert OpCode.OP_MULTIPLY_NUMBER in bytecode
    assert OpCode.OP_MULTIPLY not in bytecode

def test_number_division_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: number = 6; var b: number = 3; var c = a / b;")
...
def test_number_equality_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: number = 2; var b: number = 2; var c = a == b;")
    assert OpCode.OP_EQUAL_NUMBER in bytecode
    assert OpCode.OP_EQUAL not in bytecode

def test_number_negation_bytecode():
    bytecode, _ = compile_and_get_bytecode_and_constants("var a: number = 1; var b = -a;")
    assert OpCode.OP_NEGATE_NUMBER in bytecode
    assert OpCode.OP_NEGATE not in bytecode

def test_constant_folding_binary():
    bytecode, constants = compile_and_get_bytecode_and_constants("var x = 2 * 3;")
    assert 6 in constants
    assert OpCode.OP_MULTIPLY_NUMBER not in bytecode
    assert OpCode.OP_MULTIPLY not in bytecode

def test_constant_folding_unary():
    bytecode, constants = compile_and_get_bytecode_and_constants("var x = -5;")
    assert -5 in constants
    assert OpCode.OP_NEGATE_NUMBER not in bytecode
    assert OpCode.OP_NEGATE not in bytecode
